...
class ThePayloadIsTheAllowlistAndNothingElse(unittest.TestCase):
    def test_it_carries_exactly_the_three_allowed_fields(self):
        self.assertEqual(
            sorted(provenance_of(a_manifest())), sorted(ALLOWED_PROVENANCE)
        )

    def test_the_digest_is_the_manifest_that_produced_the_file(self):
        manifest = a_manifest()
        self.assertEqual(provenance_of(manifest)["manifest_digest"], manifest.digest())

    def test_the_parameters_are_not_in_it(self):
    ...
    def test_one_extra_field_is_refused(self):
    ...
    def test_a_field_nobody_named_is_refused_too(self):
    ...
    def test_a_payload_missing_a_field_is_refused(self):
        payload = provenance_of(a_manifest())
        del payload["seeds"]
        with self.assertRaises(PublishRefused) as refusal:
            refuse_anything_outside_the_allowlist(payload)
    ...
    def test_the_allowlisted_payload_passes(self):
...
class ItSaysWhatItWouldSendBeforeItSendsIt(unittest.TestCase):
    ...
    def test_the_preview_names_every_field_that_would_go(self):
        payload = provenance_of(a_manifest())
        text = describe(self.path, payload)
        for field in ALLOWED_PROVENANCE:
            self.assertIn(field, text)

    def test_the_preview_carries_the_values_and_not_only_the_names(self):
    ...
    def test_the_preview_names_what_is_excluded(self):
        text = describe(self.path, provenance_of(a_manifest()))
        for excluded in NAMED_EXCLUSIONS:
            self.assertIn(excluded, text)

    def test_the_preview_is_printed_before_the_question_is_asked(self):
...
class ItStopsUnlessTheOperatorSaysYes(unittest.TestCase):
    ...
    def test_a_no_reaches_no_sender(self):
        recorder = Recorder(answer="no")
        code = publish(
    ...
    def test_an_empty_answer_reaches_no_sender(self):
    ...
    def test_a_y_is_not_a_yes(self):
        recorder = Recorder(answer="y")
        publish(self.path, ask=recorder.ask, out=recorder.out, sender=recorder.send)
        self.assertEqual(recorder.sent, [])

    def test_a_yes_reaches_the_sender_with_the_allowlisted_payload(self):
    ...
    def test_a_yes_with_surrounding_space_and_capitals_is_a_yes(self):
        recorder = Recorder(answer="  YES \n")
        publish(self.path, ask=recorder.ask, out=recorder.out, sender=recorder.send)
        self.assertEqual(len(recorder.sent), 1)
...
class TheSeamRefuses(unittest.TestCase):
    ...
    def test_the_real_sender_refuses_rather_than_quietly_doing_nothing(self):
        with self.assertRaises(PublishRefused) as refusal:
    ...
    def test_the_seam_checks_the_allowlist_again_at_the_last_moment(self):
    ...
    def test_the_default_sender_is_the_real_one(self):
...
class ItIsReachableFromNoOtherModule(unittest.TestCase):
    ...
    def test_nothing_else_in_the_package_imports_it(self):
        import ast
