...
from tools.docs_format import Defect, inspect, repair


def rules(defects: list[Defect]) -> list[str]:
...
class TheCarriageReturnIsRefused(unittest.TestCase):
    def test_a_stored_carriage_return_is_a_defect(self) -> None:
        found = inspect("d.md", b"one\r\ntwo\n")
        self.assertEqual(rules(found), ["carriage-return"])

    def test_the_same_document_without_it_is_clean(self) -> None:
        self.assertEqual(inspect("d.md", b"one\ntwo\n"), [])

    def test_the_defect_names_the_line_it_is_on(self) -> None:
        found = inspect("d.md", b"one\ntwo\r\nthree\n")
        self.assertEqual(found[0].line, 2)

    def test_a_lone_carriage_return_is_refused_too(self) -> None:
...
class TrailingWhitespaceIsRefused(unittest.TestCase):
    def test_a_line_ending_in_a_space_is_a_defect(self) -> None:
        found = inspect("d.md", b"one \ntwo\n")
        self.assertEqual(rules(found), ["trailing-whitespace"])

    def test_a_line_with_the_space_in_the_middle_is_clean(self) -> None:
        self.assertEqual(inspect("d.md", b"one two\n"), [])

    def test_the_empty_line_after_the_final_newline_is_not_a_defect(self) -> None:
    ...
    def test_a_crlf_line_is_reported_once_and_not_twice(self) -> None:
    ...
    def test_a_line_with_both_is_reported_under_both(self) -> None:
        found = inspect("d.md", b"one \r\ntwo\n")
        self.assertEqual(
            sorted(rules(found)), ["carriage-return", "trailing-whitespace"]
        )

    def test_a_blank_line_carrying_a_space_is_a_defect(self) -> None:
        found = inspect("d.md", b"one\n \ntwo\n")
        self.assertEqual(rules(found), ["trailing-whitespace"])
        self.assertEqual(found[0].line, 2)
...
class TheFinalNewlineIsRequired(unittest.TestCase):
    def test_a_file_not_ending_in_a_newline_is_a_defect(self) -> None:
        found = inspect("d.md", b"one\ntwo")
        self.assertEqual(rules(found), ["no-final-newline"])

    def test_the_same_file_with_one_is_clean(self) -> None:
        self.assertEqual(inspect("d.md", b"one\ntwo\n"), [])

    def test_an_empty_file_is_not_reported(self) -> None:
...
class TheHardTabIsRefused(unittest.TestCase):
    def test_a_tab_is_a_defect(self) -> None:
        found = inspect("d.md", b"one\n\tindented\n")
        self.assertEqual(rules(found), ["hard-tab"])

    def test_spaces_in_the_same_position_are_clean(self) -> None:
        self.assertEqual(inspect("d.md", b"one\n    indented\n"), [])
...
class TheRepairIsTheOneTheMessageNames(unittest.TestCase):
    ...
    def test_it_repairs_the_three_it_claims(self) -> None:
        broken = b"one \r\n\ttwo  \r\nthree"
        self.assertEqual(repair(broken), b"one\n\ttwo\nthree\n")

    def test_a_repaired_document_passes_the_check_apart_from_the_tab(self) -> None:
        broken = b"one \r\ntwo  \r\nthree"
        self.assertEqual(inspect("d.md", repair(broken)), [])

    def test_the_tab_survives_the_repair(self) -> None:
    ...
    def test_a_clean_document_is_returned_unchanged(self) -> None:
        clean = b"one\ntwo\n"
