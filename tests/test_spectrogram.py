...
def an_energy_axis(points=ENERGIES):
    return np.linspace(90.0, 110.0, points)


def a_delay_axis(points=DELAYS):
    return np.linspace(-200.0, 200.0, points)


def counts(shape=(ENERGIES, DELAYS)):
    return np.ones(shape)


def a_spectrogram(**overrides):
    arguments = {
        "counts": counts(),
...
class TheShapeMustAgreeWithTheAxes(unittest.TestCase):
    ...
    def test_an_array_with_too_few_delay_columns_is_refused(self):
        with self.assertRaises(SpectrogramRefused) as refusal:
    ...
    def test_the_matching_shape_is_accepted(self):
...
class AnAxisMustBeUniform(unittest.TestCase):
    ...
    def test_a_non_uniform_delay_axis_is_refused(self):
        axis = a_delay_axis()
        axis[1] += 5.0
        with self.assertRaises(SpectrogramRefused) as refusal:
    ...
    def test_floating_point_roughness_is_accepted(self):
    ...
    def test_a_two_dimensional_axis_is_refused(self):
    ...
    def test_the_same_axis_flattened_is_accepted(self):
    ...
    def test_an_axis_with_one_point_is_refused(self):
        with self.assertRaises(SpectrogramRefused) as refusal:
            a_spectrogram(
...
class TheEnergyAxisIsInElectronvolts(unittest.TestCase):
    ...
    def test_an_axis_starting_at_zero_is_accepted(self):
    ...
    def test_an_axis_in_another_unit_is_not_caught(self):
...
class TheValuesMustBeCountsThatCanExist(unittest.TestCase):
    def test_a_negative_expected_count_is_refused(self):
        array = counts()
        array[1, 2] = -1.0
        with self.assertRaises(SpectrogramRefused) as refusal:
    ...
    def test_a_zero_count_is_accepted(self):
    ...
    def test_an_axis_value_that_is_not_a_number_is_refused(self):
        axis = a_delay_axis()
        axis[0] = np.inf
        with self.assertRaises(SpectrogramRefused) as refusal:
            a_spectrogram(delay_axis_attosecond=axis)
...
class TheOperationsEverythingDownstreamNeeds(unittest.TestCase):
    ...
    def test_a_window_of_one_bin_is_one_bin(self):
    ...
    def test_the_window_applies_to_the_counts_and_the_axis_alike(self):
        window = self.spectrogram.energy_window(102.0, 106.0)
        self.assertEqual(
    ...
    def test_the_total_is_every_expected_count(self):
